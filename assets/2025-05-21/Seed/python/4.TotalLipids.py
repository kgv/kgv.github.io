...
# Константа для массы стандарта
MG_PER_100_UL_STANDARD = 0.025 # мг

def calculate_total_mass_from_gc_table(table_string: str, mg_per_100ul: float = MG_PER_100_UL_STANDARD) -> dict:
    """
    Парсит ОДНУ таблицу данных ГХ, вычисляет сумму площадей веществ (кроме 17:0),
    делит её на площадь 17:0 и умножает на массу 17:0.
    Возвращает словарь с промежуточными и конечным результатами.
    """
    table_string_cleaned = table_string.strip()
    if not table_string_cleaned:
        raise ValueError("Предоставлена пустая строка для таблицы.")
        
    lines = table_string_cleaned.split('\n')

    if len(lines) < 2:
        raise ValueError("Строка таблицы слишком короткая. Должна содержать как минимум строку заголовка и строку с названиями столбцов.")

    title_line = lines[0]
    match_volume = re.search(r'\((\d+(?:\.\d+)?)\s*uL\s*17:0\)', title_line)
    if not match_volume:
        raise ValueError(f"Не удалось найти объем 17:0 в строке заголовка: '{title_line}'")
    volume_17_0_uL = float(match_volume.group(1))

    mass_17_0_mg = (volume_17_0_uL / 100.0) * mg_per_100ul

    sum_other_areas = 0.0
    area_17_0 = 0.0
    found_17_0 = False
    
    column_header_line_raw = lines[1]
    column_header_line_processed = column_header_line_raw.strip().strip('|')
    headers = [h.strip() for h in column_header_line_processed.split('|')]

    try:
        name_col_label = "Обозначение"
        area_col_label = "Площадь (мВ*с)"
        
        name_col_idx = headers.index(name_col_label)
        area_col_idx = headers.index(area_col_label)
    except ValueError:
        if len(lines) <=2:
             raise ValueError(f"Отсутствуют строки данных в таблице после заголовков. Найденные заголовки: {headers}")
        raise ValueError(
            f"Необходимые столбцы '{name_col_label}' или '{area_col_label}' не найдены в заголовках таблицы. "
            f"Найденные заголовки: {headers}"
        )

    if len(lines) <= 2:
        raise ValueError("В таблице отсутствуют строки данных после заголовков.")

    for i, data_line_raw in enumerate(lines[2:]):
        data_line_processed = data_line_raw.strip()
        if not data_line_processed or not data_line_processed.startswith('|'):
            continue
        
        cells_raw = data_line_processed.strip('|')
        cells = [cell.strip() for cell in cells_raw.split('|')]

        if len(cells) <= max(name_col_idx, area_col_idx):
            continue
            
        substance_name = cells[name_col_idx]
        area_val_str = cells[area_col_idx]

        try:
            current_area = float(area_val_str) if area_val_str else 0.0
        except ValueError:
            continue

        if substance_name == "17:0":
            area_17_0 = current_area
            found_17_0 = True
        else:
            sum_other_areas += current_area
            
    if not found_17_0:
        raise ValueError("Вещество '17:0' не найдено в данных таблицы.")
    if area_17_0 == 0:
        raise ValueError("Площадь '17:0' равна нулю, что приведет к делению на ноль.")

    calculated_total_mass = (sum_other_areas / area_17_0) * mass_17_0_mg
    
    return {
        'final_result': calculated_total_mass,
        'sum_other_areas': sum_other_areas,
        'area_17_0': area_17_0,
        'mass_17_0_mg': mass_17_0_mg
    }

def extract_grouping_key_and_display_name(title_line: str) -> tuple[str, str]:
    """
    Извлекает ключ группировки (основной заголовок) и отображаемое имя для группы.
    Ключ группировки и отображаемое имя группы: основной_заголовок (часть до первого '[' или '(').
    """
    idx_bracket = title_line.find('[')
    idx_parenthesis = title_line.find('(')

    end_indices = []
    if idx_bracket != -1:
        end_indices.append(idx_bracket)
    if idx_parenthesis != -1:
        end_indices.append(idx_parenthesis)

    if not end_indices: 
        base_title_for_grouping = title_line.strip()
    else:
        base_title_for_grouping = title_line[:min(end_indices)].strip()
    
    return base_title_for_grouping, base_title_for_grouping


# Ваши тестовые данные (уменьшенный набор)
all_tables_data = """
...
.Семена(10 мм) {2024-4-07} [1] (600 uL 17:0)
| Обозначение | Компонент                                            | Время (мин)| Относительное время| Площадь (мВ*с)| Площадь (%)
...
.Семена(10 мм) {2024-4-07} [2] (600 uL 17:0)
| Обозначение | Компонент                                            | Время (мин)| Относительное время| Площадь (мВ*с)| Площадь (%)
| 14:0        | Methyl tetradecanoate                                | 0.000      | 0.000              | 0.000         | 0.000
...
.Семена(10 мм) {2024-4-07} [3] (600 uL 17:0)
| Обозначение | Компонент                                            | Время (мин)| Относительное время| Площадь (мВ*с)| Площадь (%)
...
.Семена(10 мм) {2024-4-07} [4] (600 uL 17:0)
| Обозначение | Компонент                                            | Время (мин)| Относительное время| Площадь (мВ*с)| Площадь (%)
| 14:0        | Methyl tetradecanoate                                | 0.000      | 0.000              | 0.000         | 0.000
...
table_strings_list = re.split(r'\n\s*\n+', all_tables_data.strip())
results_by_group = defaultdict(lambda: {'values': [], 'display_header': ''})
processing_log = [] 

print("--- Индивидуальные расчеты для каждой таблицы ---")
for i, single_table_str in enumerate(table_strings_list):
    if not single_table_str.strip():
        continue

    full_title_line = single_table_str.strip().split('\n', 1)[0]
    group_key_for_stats, display_name_for_stats_group = extract_grouping_key_and_display_name(full_title_line)
    
    log_entry = {'full_title': full_title_line, 'group_key': group_key_for_stats}
    
    print(f"\nОбработка таблицы: {full_title_line}")
    
    try:
        calc_details = calculate_total_mass_from_gc_table(single_table_str)
        result = calc_details['final_result']
        
        # Вывод с промежуточными вычислениями
        print(f"  ({calc_details['sum_other_areas']:.3f} / {calc_details['area_17_0']:.3f}) * {calc_details['mass_17_0_mg']:.5f} = {result:.8f} мг")
        
        results_by_group[group_key_for_stats]['values'].append(result)
        results_by_group[group_key_for_stats]['display_header'] = display_name_for_stats_group
...
for group_key_stats, group_data in results_by_group.items():
    values = group_data['values']
    group_display_name = group_data['display_header']
    
    print(f"\nГруппа: {group_display_name}")
    
    valid_sources_in_group = [log['full_title'] for log in processing_log if log['group_key'] == group_key_stats and log['result'] is not None]
    if valid_sources_in_group:
        print(f"  Источники данных (валидные): {', '.join(valid_sources_in_group)}")
    
    if not values:
        print("  Нет валидных результатов для расчета статистики.")
        errors_in_group = [f"{log['full_title']}: {log['error']}" for log in processing_log if log['group_key'] == group_key_stats and log['error']]
        if errors_in_group:
            print(f"  Обнаруженные ошибки в этой группе: {'; '.join(errors_in_group)}")
        continue

    print(f"  Количество валидных результатов: {len(values)}")
    print(f"  Значения (мг): {[f'{v:.4f}' for v in values]}")

    mean_val = np.mean(values)
    
    if len(values) > 1:
        std_val = np.std(values, ddof=1)
        print(f"  Результат группы: {mean_val:.4f} ±{std_val:.4f} мг")
    else:
        print(f"  Результат группы: {mean_val:.4f} мг")